...
- [ ] Phase 1(1~30일): 보안/관측성 기본선 구현.
- [ ] Phase 2(31~60일): Durable Job 아키텍처 전환(인프로세스 작업러너 제거).
- [ ] Phase 3(61~90일): 실험/데이터 운영 표준화 및 SLO 운영 고도화.
...
현재 문서는 구현 시작 전 기준선 계획이다. 결과 섹션은 각 Phase 종료 시 반드시 갱신한다. 최소 기록 항목은 다음과 같다. 실제 장애 감소 지표(5xx, 작업 실패율), 성능 지표(P95 작업 시작 지연, 평균 처리시간), 품질 지표(평가 run 재현률), 운영 지표(탐지-복구 시간)다.
...
모든 검증은 저장소 루트(`/Users/chsong/Developer/Personal/kra-analysis`)에서 실행한다.
...
- `apps/api/services/job_service.py`는 구체 구현을 직접 호출하지 않고 `JobRunner` 인터페이스를 의존해야 한다.
    ...
    class JobRunner(Protocol):
        async def submit(self, job_type: str, params: dict[str, Any], job_id: str) -> str: ...
...
- `packages/scripts/evaluation/run_metadata.py`는 평가/개선 런타임에서 공통으로 사용할 메타데이터 생성기 함수를 제공해야 한다.
    ...
    def build_run_metadata(prompt_version: str, dataset_id: str, mode: str) -> dict[str, Any]: ...
