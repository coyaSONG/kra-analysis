...
```bash
# Monorepo 기준 실제 구조 (요약)
apps/api/
├─ main_v2.py            # FastAPI 진입점 (v2)
├─ config.py             # 설정 관리
├─ dependencies/         # 인증 등 의존성
├─ infrastructure/       # DB/Redis/Celery/Supabase 클라이언트
├─ middleware/           # 로깅/레이트리밋
├─ models/               # DTO/ORM 모델
├─ routers/              # 라우터 (collection_v2.py, jobs_v2.py)
├─ services/             # 도메인 서비스
├─ tasks/                # 비동기 작업 (collection_tasks.py)
└─ tests/                # unit/integration
```

#### 1.2 의존성 설치

> 실제 의존성은 `apps/api/pyproject.toml`에 정의되어 있습니다. 수동 설치 대신 `uv sync` 사용을 권장합니다.

#### 1.3 환경 설정

`.env` 예시는 `apps/api/.env.example`를 참고하세요. 개발 기본 포트는 8001입니다.
...
```python
# services/kra_api_service.py (요약)
import httpx
...
##### Step 1: 병렬 운영 준비
```bash
# 1. 새 API 서버 실행 (앱 디렉터리에서)
uv run uvicorn main_v2:app --reload --port 8001

# 2. Celery 워커 실행
celery -A infrastructure.celery_app.celery_app worker --loglevel=info

# 3. Celery Beat 실행 (스케줄링)
celery -A infrastructure.celery_app.celery_app beat --loglevel=info
```
...
class KRAApiUser(HttpUser):
    ...
    def get_races(self):
        self.client.get(
            "/api/v2/collection/status?date=20250622&meet=1",
            headers={"X-API-Key": "test_key"}
        )
...
# 실행
CMD ["uvicorn", "main_v2:app", "--host", "0.0.0.0", "--port", "8001"]
```
...
services:
  ...
  celery-worker:
    build: .
    command: celery -A infrastructure.celery_app.celery_app worker --loglevel=info
    environment:
      - DATABASE_URL=postgresql://kra:password@postgres:5432/kra_db
      - REDIS_URL=redis://redis:6379
    depends_on:
  ...
  celery-beat:
    build: .
    command: celery -A infrastructure.celery_app.celery_app beat --loglevel=info
    environment:
      - DATABASE_URL=postgresql://kra:password@postgres:5432/kra_db
      - REDIS_URL=redis://redis:6379
    depends_on:
...
1. **Celery 작업이 실행되지 않음**
   ```bash
   # 워커 상태 확인
   celery -A infrastructure.celery_app.celery_app inspect active
   
   # 큐 상태 확인
   celery -A infrastructure.celery_app.celery_app inspect reserved
   ```
