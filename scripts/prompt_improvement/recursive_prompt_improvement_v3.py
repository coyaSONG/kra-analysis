    ...
    def analyze_results(self, current_results: Dict, previous_results: Dict = None) -> Dict:
        ...
        analysis = {
            "current_performance": {
                ...
                "total_races": current_results["total_races"],
...
- 신마/데이터 부족: 배당률만으로 평가"""
    ...
    def generate_final_report(self):
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            # 성능 변화 표
            f.write("## 성능 변화 추이\n\n")
            f.write("| Iteration | Version | 경주 수 | 적중률 | 평균 적중 | 에러율 | 변화 |\n")
            f.write("|-----------|---------|---------|--------|----------|--------|------|\n")
        ...
        if self.iteration_history:
            ...
            if best_prompt.exists():
                final_prompt = Path("prompts") / f"prediction-template-optimized-v3.md"
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python recursive_prompt_improvement_v3.py <base_prompt_file> [date_filter] [max_iterations] [max_workers]")
        print("\nExamples:")
        print("  모든 경주: python recursive_prompt_improvement_v3.py prompts/prediction-template-v10.3.md all 5 3")
        print("  특정 날짜: python recursive_prompt_improvement_v3.py prompts/prediction-template-v10.3.md 20250601 5 3")
        print("\n사용 가능한 날짜:")
        
        # 사용 가능한 날짜 표시
        improver = RecursivePromptImproverV3("dummy")
    ...
    # 재귀 개선 실행
    improver = RecursivePromptImproverV3(base_prompt, max_iterations)
