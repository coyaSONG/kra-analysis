...
def fetch_race_result_api214(meet: str, rc_date: str, rc_no: int) -> Optional[Dict]:
    """API214_1을 통해 경주 결과 가져오기"""
    # meet 코드 매핑
    meet_codes = {
        '서울': '1',
...
def save_result_cache(result_data: Dict, meet: str, rc_date: str, rc_no: int, 
...
def load_result_cache(meet: str, rc_date: str, rc_no: int, 
                     cache_dir: Path = Path("data/cache/results")) -> Optional[Dict]:
    """캐시에서 결과 로드"""
    meet_codes = {'서울': '1', '제주': '2', '부산경남': '3'}
    meet_code = meet_codes.get(meet, '1')
...
def get_race_result_with_cache(meet: str, rc_date: str, rc_no: int) -> Optional[List[int]]:
