...
class PredictionTester:
    ...
    def find_enriched_files(self, date_filter: Optional[str] = None) -> List[Dict[str, any]]:
        """enriched 파일 찾기"""
        enriched_files = []
    ...
    def load_race_data(self, file_info: Dict) -> Optional[Dict]:
    ...
    def run_prediction(self, race_data: Dict, race_id: str) -> Optional[Dict]:
        """Claude를 사용하여 예측 수행"""
        try:
            # 프롬프트 읽기
    ...
    def run_test(self, date_filter: Optional[str] = None, limit: Optional[int] = None):
        """예측 테스트 실행"""
        print(f"\n{'='*60}")
        print(f"경주 예측 테스트 시작")
    ...
    def save_results(self, predictions: List[Dict], analyses: List[Dict], date_filter: Optional[str]):
        """예측 결과 저장"""
