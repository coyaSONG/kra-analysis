...
class CollectionRequest(BaseModel):
    ...
    def validate_date(cls, v):
        ...
        try:
            ...
            raise ValueError(f"validation error: 유효하지 않은 날짜: {e}") from e
        return v

    meet: int = Field(
        ..., ge=1, le=3, description="경마장 (1: 서울, 2: 제주, 3: 부산경남)"
    )
    race_numbers: list[int] | None = Field(
        None, description="수집할 경주 번호 리스트. 생략시 1-15R 전체"
    )
    options: CollectionOptions | None = Field(None, description="추가 옵션")
...
class CollectionResponse(BaseModel):
    ...
    job_id: str | None = Field(None, description="작업 ID")
    status: str = Field(..., description="작업 상태")
    message: str = Field(..., description="응답 메시지")
    estimated_time: int | None = Field(None, description="예상 소요 시간(초)")
    webhook_url: str | None = Field(None, description="상태 확인 URL")
    data: list[dict[str, Any]] | None = Field(None, description="수집된 데이터")
...
class CollectionStatus(BaseModel):
    ...
    collection_status: DataStatus | None = Field(None, description="수집 상태")
    enrichment_status: DataStatus | None = Field(None, description="보강 상태")
    result_status: DataStatus | None = Field(None, description="결과 상태")
...
class HorseData(BaseModel):
    ...
    rating: int | None = Field(None, description="레이팅")

    # 보강 데이터
    hrDetail: dict[str, Any] | None = Field(None, description="말 상세 정보")
    jkDetail: dict[str, Any] | None = Field(None, description="기수 상세 정보")
    trDetail: dict[str, Any] | None = Field(None, description="조교사 상세 정보")
...
class RaceInfo(BaseModel):
    ...
    track: str | None = Field(None, description="주로")
    weather: str | None = Field(None, description="날씨")
    meet: int = Field(..., description="경마장")
...
class RaceData(BaseModel):
    ...
    enrichment_status: DataStatus | None = Field(None, description="보강 상태")
    result_status: DataStatus | None = Field(None, description="결과 수집 상태")

    # 결과 데이터
    result: dict[str, Any] | None = Field(None, description="경주 결과")

    # 타임스탬프
    collected_at: datetime = Field(..., description="수집 시간")
    enriched_at: datetime | None = Field(None, description="보강 시간")
    result_collected_at: datetime | None = Field(None, description="결과 수집 시간")

    # 메타데이터
    data_quality_score: float | None = Field(
        None, ge=0, le=1, description="데이터 품질 점수"
    )
