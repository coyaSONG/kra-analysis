...
class NodeJsPythonComparison:
    ...
    async def run_nodejs_script(self, script_name: str, args: List[str]) -> Dict[str, Any]:
    ...
    def load_nodejs_race_data(self, date: str, meet: int, race_no: int, file_type: str = "prerace") -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            file_path = self.data_path / "races" / year / month / date / meet_folder / f"race_{meet}_{date}_{race_no}_{file_type}.json"
    ...
    async def test_race_detail_collection(self, date: str = "20250106", meet: int = 1, race_no: int = 1):
        """경주 상세 데이터 수집을 비교 테스트합니다."""
        print(f"\n{'='*60}")
        print(f"경주 상세 데이터 수집 테스트")
    ...
    def compare_race_data(self, nodejs_data: Dict[str, Any], python_data: Dict[str, Any]):
        """Node.js와 Python 데이터를 비교합니다."""
        print("\n[기본 정보 비교]")
        
        # 경주 정보 비교
        fields_to_compare = ["date", "meet", "race_no", "race_name", "distance", "track_condition", "weather"]
        ...
        if len(nodejs_horses) == len(python_horses):
            ...
            if nodejs_horses and python_horses:
                print("\n[첫 번째 말 상세 비교]")
                horse_fields = ["chul_no", "horse_no", "horse_name", "jockey_name", "trainer_name", "win_odds"]
    ...
    async def test_api_endpoints(self):
        ...
        print(f"\n{'='*60}")
        print(f"FastAPI 엔드포인트 테스트")
        print(f"{'='*60}\n")
        
        async with httpx.AsyncClient() as client:
            # 1. 헬스체크
            print("1. 헬스체크...")
            response = await client.get(f"{self.api_base_url.replace('/api/v1', '')}/health")
            ...
            response = await client.post(f"{self.api_base_url}/races/collect", json=collect_data)
            print(f"상태: {response.status_code}")
            ...
            if response.status_code == 200:
                ...
                print("\n3. 수집 상태 확인...")
                response = await client.get(f"{self.api_base_url}/races/{job_id}/status")
                print(f"상태: {response.status_code}")
                print(f"응답: {response.json()}")
    
    async def test_enrichment_comparison(self, date: str = "20250106", meet: int = 1, race_no: int = 1):
        """데이터 보강(enrichment) 비교 테스트입니다."""
        print(f"\n{'='*60}")
        print(f"데이터 보강(Enrichment) 비교 테스트")
        print(f"날짜: {date}, 경마장: {meet}, 경주: {race_no}")
        print(f"{'='*60}\n")
