...
class TestCollectionEndpoints:
    ...
    async def test_collect_races_invalid_date(self, authenticated_client: AsyncClient):
        ...
        response = await authenticated_client.post(
            "/api/v2/collection/",
            json={"date": "2024-07-19", "meet": 1, "race_numbers": [1]},  # Wrong format
        )
    ...
    async def test_collect_races_future_date(self, authenticated_client: AsyncClient):
        ...
        response = await authenticated_client.post(
            "/api/v2/collection/",
            json={"date": future_date, "meet": 1, "race_numbers": [1]},
        )
    ...
    @pytest.mark.integration
    async def test_get_collection_status_endpoint(
        self, authenticated_client: AsyncClient
    ):
        r = await authenticated_client.get(
            "/api/v2/collection/status?date=20240719&meet=1"
        )
...
class TestAsyncCollectionEndpoints:
    ...
    async def test_async_collect_races(self, authenticated_client: AsyncClient):
        ...
        with patch(
            "routers.collection_v2.job_service.job_runner.submit",
...
class TestAuthenticationEndpoints:
    ...
    async def test_invalid_api_key(self, client: AsyncClient):
        ...
        response = await client.get(
            "/api/v2/jobs/", headers={"X-API-Key": "invalid-key"}
        )
