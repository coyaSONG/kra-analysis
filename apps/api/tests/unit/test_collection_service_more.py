...
async def test_save_race_data_update_path(db_session):
    ...
    r = Race(
        ...
        date='20240719', race_date='20240719', meet=1, race_number=1, race_no=1,
        basic_data={'init': True}, status=DataStatus.PENDING, collection_status=DataStatus.PENDING
    )
    db_session.add(r)
    await db_session.commit()

    data = {
        'date': '20240719', 'meet': 1, 'race_number': 1,
        'race_date': '20240719', 'race_no': 1
    }
