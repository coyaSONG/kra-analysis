...
from models.database_models import Prediction, Job
from models.database_models import JobStatus, JobType
...
async def test_require_resource_access_job_allow_and_deny(db_session):
    ...
    dbkey = DBAPIKey(key='test-api-key-123', name='Environment Key', is_active=True, permissions=['read','write'])
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url='http://test') as ac:
        ...
        r_ok = await ac.get(f'/guard/jobs/{j.job_id}', headers={'X-API-Key': 'test-api-key-123'})
        assert r_ok.status_code == 200

        # Deny: wrong job id
        r_dn = await ac.get('/guard/jobs/does-not-exist', headers={'X-API-Key': 'test-api-key-123'})
