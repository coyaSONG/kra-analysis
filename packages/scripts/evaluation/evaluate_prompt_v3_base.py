...
class PromptEvaluatorV3Base:
    ...
    def load_race_data(self, enriched_file: Path) -> Optional[Dict]:
    ...
    def run_claude_prediction(self, race_data: Dict, race_id: str) -> Tuple[Optional[Dict], str]:
        """Claude를 사용하여 예측 수행"""
        try:
            # 프롬프트 읽기
    ...
    def _parse_stream_json(self, output: str, execution_time: float) -> Optional[Dict]:
        """stream-json 형식 파싱"""
        try:
    ...
    def _parse_regular_output(self, output: str, execution_time: float) -> Optional[Dict]:
        """일반 출력 파싱 (폴백)"""
        try:
            # 코드블록 내 JSON
