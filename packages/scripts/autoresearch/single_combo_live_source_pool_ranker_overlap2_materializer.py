"""Materialize or strictly block live source-pool ranker after overlap2 source."""
...
DEFAULT_SOURCE_TARGET = DEFAULT_CACHE_DIR / (
    "clean_release_current_best_all_allowed_source_pool_ranker_after_"
    "overlap2_aggregate_prior_date_selector_rerun_repro_diagnostic.json"
)
DEFAULT_LIVE_SOURCE = (
    DEFAULT_CACHE_DIR
...
PARENT_ACTION = "port_locked_best_source_pool_ranker_overlap2_source_to_live_runner"
CHILD_ACTION = "port_locked_best_source_pool_overlap2_aggregate_source_to_live_runner"
    ...
    *,
    ...
    if live_source_context.get("status") != "passed":
        ...
        if isinstance(child_action, str) and child_action:
            return {
                ...
                "queue_priority_score": float(
                    child_recommended.get("queue_priority_score", 94.78)
                ),
                "reason": (
                    child_recommended.get("reason")
                    or "The required source-pool overlap2 source reported a downstream live-port dependency."
                ),
        ...
        return {
            ...
            "reason": (
                "The source-pool ranker after overlap2 live source cannot emit "
                "predictions until the all-allowed source-pool overlap2 "
                "aggregate after source-meta-extension source is materialized "
                "with the exact contract."
            ),
        }
    if candidate_status != "passed":
        return {
            "action": "repair_live_candidate_features_before_source_pool_ranker_overlap2_port",
            "blocking": False,
            "classification": "background_modeling_candidate",
            "queue_priority_score": 94.76,
            "reason": (
                "The source-pool ranker needs complete live full-combo candidate "
                "features and live records before pass-through can be trusted."
            ),
        }
    if ready_to_pass_through:
        return {
            "action": "materialize_locked_best_row_cache_rank_pattern_after_source_pool_from_passed_source_pool_ranker",
            "blocking": False,
            "classification": "background_modeling_candidate",
            "queue_priority_score": 94.74,
            "reason": (
                "The fallback-only source-pool ranker has been materialized and "
                "can now unblock its row-cache parent."
            ),
        }
    return {
        "action": "inspect_source_pool_ranker_overlap2_pass_through_coverage_gap",
        "blocking": False,
        "classification": "background_modeling_candidate",
        "queue_priority_score": 94.72,
        "reason": (
            "The source-pool ranker source is present, but its live prediction "
...
def build_artifact(
    *,
    ...
    if ready_to_pass_through:
        ...
        status = "blocked_source_pool_ranker_overlap2_coverage_gap"
        coverage = {
            "status": "blocked",
            "expected_race_count": expected_race_count,
            "predicted_race_count": predicted_race_count,
        ...
        predictions = {}
    return {
        "format_version": "single-combo-live-source-pool-ranker-overlap2-materializer-v1",
