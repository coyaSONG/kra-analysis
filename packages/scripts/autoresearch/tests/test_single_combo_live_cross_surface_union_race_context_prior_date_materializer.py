"""Live cross-surface union race-context prior-date materializer tests."""
...
from autoresearch import (  # noqa: E402
    single_combo_live_cross_surface_union_race_context_prior_date_materializer as materializer,
)
...
def _write_source_target(path: Path) -> None:
    _write_json(
        ...
        {
            "best": {
                "candidate": "clean_current_best_cross_surface_union_prior_date/fallback_only",
                "selector_spec": "fallback_only",
                "summary": {
                    "overfit_safe_exact_rate": 0.538462,
                    "test_exact_3of3_rate": 0.55102,
                    "robust_pool_oracle_exact_rate": 0.538462,
                },
                "windows": [
                    {
                        "name": "test",
                        "diagnostics": {
                            "feature_contract": "current_best_fallback_only",
                            "history_update": "none",
            ...
            "selection_contract": "clean_current_best_cross_surface_union_prior_date",
            "source_artifact": "offline-broad-component-first-exact-rank.json",
        },
    )
...
def test_build_artifact_blocks_when_broad_component_source_is_missing(
    tmp_path: Path,
) -> None:
    source_target = tmp_path / "cross-surface-target.json"
    ...
    result = materializer.build_artifact(
        ...
        live_source_path=tmp_path / "missing-broad-component.json",
...
def test_build_artifact_follows_broad_component_child_action(tmp_path: Path) -> None:
    source_target = tmp_path / "cross-surface-target.json"
    candidates = tmp_path / "candidates.json"
    source = tmp_path / "broad-component-source.json"
    _write_source_target(source_target)
    ...
    _write_json(
        source,
        {
            ...
            "recommended_next_action": {
                ...
                "reason": "broad-component first-exact rank model scoring is pending",
            },
...
def test_build_artifact_passes_through_fallback_only_source(tmp_path: Path) -> None:
    ...
    assert result["recommended_next_action"]["action"] == (
        "materialize_locked_best_entity_history_overlay_from_passed_cross_surface_union_race_context"
    )
