"""Live entity-history pair-overlay prior-date materializer tests."""
...
from autoresearch import (  # noqa: E402
    single_combo_live_entity_history_pair_overlay_prior_date_materializer as materializer,
)
...
def _write_source_target(path: Path) -> None:
    _write_json(
        ...
        {
            "best": {
                "candidate": "clean_current_best_entity_history_pair_overlay/fallback_only",
                "selector_spec": "fallback_only",
                "summary": {
                    "overfit_safe_exact_rate": 0.538462,
                    "test_exact_3of3_rate": 0.55102,
                    "robust_pair_pool_oracle_exact_rate": 0.538462,
                },
                "windows": [
                    {
                        "name": "test",
            ...
            "selection_contract": "clean_current_best_entity_history_pair_overlay",
            "source_artifact": "offline-entity-history-overlay.json",
        },
    )
...
def test_build_artifact_blocks_when_entity_overlay_source_is_missing(
    tmp_path: Path,
) -> None:
    source_target = tmp_path / "pair-overlay-target.json"
    ...
    result = materializer.build_artifact(
        ...
        live_source_path=tmp_path / "missing-entity-overlay.json",
...
def test_build_artifact_follows_entity_overlay_child_action(tmp_path: Path) -> None:
    source_target = tmp_path / "pair-overlay-target.json"
    candidates = tmp_path / "candidates.json"
    source = tmp_path / "entity-overlay-source.json"
    _write_source_target(source_target)
    ...
    _write_json(
        source,
        {
            ...
            "recommended_next_action": {
                "action": "port_locked_best_cross_surface_union_race_context_prior_date_source_to_live_runner",
                "blocking": False,
                "queue_priority_score": 94.92,
                "reason": "cross-surface union race-context source is missing",
            },
...
def test_build_artifact_passes_through_fallback_only_source(tmp_path: Path) -> None:
    ...
    assert result["recommended_next_action"]["action"] == (
        "materialize_locked_best_row_cache_rank_pattern_from_passed_entity_history_pair_overlay"
    )
