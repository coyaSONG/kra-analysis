"""Live row-cache rank-pattern after-source-pool materializer tests."""
...
from autoresearch import (  # noqa: E402
    single_combo_live_row_cache_rank_pattern_after_source_pool_materializer as materializer,
)
...
def _write_source_target(path: Path) -> None:
    _write_json(
        path,
        {
            ...
            "source_artifact": "offline-source-pool-ranker-overlap2.json",
        },
    )
...
def test_build_artifact_blocks_when_source_pool_ranker_source_is_missing(
    tmp_path: Path,
) -> None:
    source_target = tmp_path / "row-cache-target.json"
    candidates = tmp_path / "candidates.json"
    missing_source = tmp_path / "missing-source-pool-ranker-overlap2.json"
...
def test_build_artifact_follows_source_pool_ranker_child_action(
    tmp_path: Path,
) -> None:
    source_target = tmp_path / "row-cache-target.json"
    candidates = tmp_path / "candidates.json"
    source = tmp_path / "source-pool-ranker-overlap2.json"
    _write_source_target(source_target)
    ...
    _write_json(
        source,
        {
            ...
            "recommended_next_action": {
                "action": "port_locked_best_source_pool_overlap2_aggregate_source_to_live_runner",
                "blocking": False,
                "queue_priority_score": 94.78,
                "reason": "source-pool overlap2 aggregate source is missing",
            },
...
def test_build_artifact_passes_through_fallback_only_source(tmp_path: Path) -> None:
    ...
    assert result["recommended_next_action"]["action"] == (
        "materialize_locked_best_broad_component_rank_segment_after_row_cache_from_passed_row_cache"
    )
    assert result["policy"]["fallback_only_source_pool_ranker_pass_through"]
