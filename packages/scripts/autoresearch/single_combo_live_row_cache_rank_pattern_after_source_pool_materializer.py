"""Materialize or strictly block live row-cache rank-pattern after source-pool source."""
...
DEFAULT_SOURCE_TARGET = DEFAULT_CACHE_DIR / (
    "clean_release_current_best_row_cache_rank_pattern_after_source_pool_"
    ...
    *,
    ...
    if live_source_context.get("status") != "passed":
        ...
        if isinstance(child_action, str) and child_action:
            return {
                ...
                "queue_priority_score": float(
                    child_recommended.get("queue_priority_score", 94.76)
                ),
                "reason": (
                    child_recommended.get("reason")
                    or "The required source-pool ranker reported a downstream live-port dependency."
                ),
                "upstream_action": (
        ...
        return {
            ...
            "reason": (
                "The row-cache rank-pattern after source-pool live source "
                "cannot emit predictions until the all-allowed source-pool "
                "ranker after overlap2 aggregate source is materialized with "
                "the exact contract."
            ),
        }
    if candidate_status != "passed":
        return {
            "action": "repair_live_candidate_features_before_row_cache_rank_pattern_after_source_pool_port",
            "blocking": False,
            "classification": "background_modeling_candidate",
    ...
    if ready_to_pass_through:
        return {
            "action": "materialize_locked_best_broad_component_rank_segment_after_row_cache_from_passed_row_cache",
            "blocking": False,
            "classification": "background_modeling_candidate",
            "queue_priority_score": 94.72,
            "reason": (
                "The fallback-only row-cache after source-pool source is "
                "materialized and can now unblock its broad-rank parent."
            ),
        }
    return {
        "action": "implement_locked_best_row_cache_rank_pattern_after_source_pool_prediction_logic",
        "blocking": False,
        "classification": "background_modeling_candidate",
        "queue_priority_score": 94.72,
        "reason": (
            "The source-pool fallback source and candidate features are "
            "present; implement pre-race row-cache rank-pattern scoring "
            "without active labels."
        ),
    }
...
def build_artifact(
    *,
    ...
    if ready_to_pass_through:
        ...
        status = (
            "blocked_pending_row_cache_rank_pattern_after_source_pool_prediction_logic"
        )
    if not ready_to_pass_through:
        ...
        predictions = {}
    return {
        "format_version": "single-combo-live-row-cache-rank-pattern-after-source-pool-materializer-v1",
        ...
        "policy": {
            "fallback_only_source_pool_ranker_pass_through": selector.get(
                "selector_spec"
            )
            == "fallback_only",
