"""Materialize or strictly block live row-cache rank-pattern after broad-rank-segment source."""
...
DEFAULT_SOURCE_TARGET = DEFAULT_CACHE_DIR / (
    "clean_release_current_best_row_cache_rank_pattern_after_broad_rank_segment_"
    "reanchor_prior_date_selector_rerun_repro_diagnostic.json"
)
DEFAULT_LIVE_SOURCE = (
    DEFAULT_CACHE_DIR
...
EXPECTED_SOURCE_SELECTION_CONTRACT = (
    "clean_current_best_plus_broad_component_rank_segment_prior_date_selector"
)
LIVE_SOURCE_SELECTION_CONTRACT = "live_current_best_plus_broad_component_rank_segment_prior_date_selector_predictions_by_window"
...
def _source_context(path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "live_selection_contract": selection_contract,
...
def _recommended_next_action(
    *,
    ...
    if live_source_context.get("status") != "passed":
        ...
        if isinstance(child_action, str) and child_action:
            return {
                ...
                "queue_priority_score": float(
                    child_recommended.get("queue_priority_score", 94.72)
                ),
                "reason": (
                    child_recommended.get("reason")
                    or "The required broad-component rank-segment source reported a downstream live-port dependency."
                ),
                "upstream_action": (
        ...
        return {
            "action": "port_locked_best_broad_component_rank_segment_after_row_cache_rank_pattern_source_to_live_runner",
            "blocking": False,
            "classification": "background_modeling_candidate",
            "queue_priority_score": 94.72,
            "reason": (
                "The row-cache rank-pattern after broad-rank-segment live source "
                "cannot emit predictions until its broad-component rank-segment "
                "after row-cache-rank-pattern fallback source is materialized "
                "with the exact contract."
            ),
        }
    if candidate_status != "passed":
        return {
            "action": "repair_live_candidate_features_before_row_cache_rank_pattern_after_broad_rank_segment_port",
            "blocking": False,
            "classification": "background_modeling_candidate",
            "queue_priority_score": 94.7,
            "reason": (
                "The row-cache source needs complete live full-combo candidate "
                "features and live records before prediction logic can run."
            ),
        }
    if ready_to_pass_through:
        return {
            "action": "materialize_locked_best_full_combo_delta_switch_after_broad_rank_segment_from_passed_row_cache",
            "blocking": False,
            "classification": "background_modeling_candidate",
            "queue_priority_score": 94.68,
            "reason": (
                "The fallback-only row-cache after broad-rank-segment source "
                "is materialized and can now unblock its full-combo delta-switch parent."
            ),
        }
    return {
        "action": "implement_locked_best_row_cache_rank_pattern_after_broad_rank_segment_prediction_logic",
        "blocking": False,
        "classification": "background_modeling_candidate",
        "queue_priority_score": 94.68,
        "reason": (
            "The broad-component rank-segment fallback source and candidate "
            "features are present; implement pre-race row-cache rank-pattern "
            "scoring without active labels."
        ),
    }
    ...
    *,
    ...
    race_ids = [
        ...
        for race_id in _list(
            candidate_coverage.get("race_ids")
            or candidate_coverage.get("expected_race_ids")
        )
    ]
    ...
    ready_to_pass_through = (
        selector.get("selector_spec") == "fallback_only"
        and source_context["status"] == "passed"
    ...
    if ready_to_pass_through:
        ...
        status = "blocked_pending_row_cache_rank_pattern_after_broad_rank_segment_prediction_logic"
    if not ready_to_pass_through:
        coverage = {
            "status": "blocked",
            "expected_race_count": expected_race_count,
        ...
        predictions = {}
    return {
        "format_version": "single-combo-live-row-cache-rank-pattern-after-broad-rank-segment-materializer-v1",
        ...
        "policy": {
            "fallback_only_broad_rank_segment_pass_through": selector.get(
                "selector_spec"
            )
            == "fallback_only",
