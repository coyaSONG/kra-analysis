...
from autoresearch import (  # noqa: E402
    clean_release_current_best_row_cache_rank_pattern_prior_date_selector_diagnostic as selector_base,
)
from autoresearch import (  # noqa: E402
    single_combo_row_cache_rank_pattern_prior_date_train_surface as train_surface,
)
...
DEFAULT_TRAIN_SURFACE = (
    DEFAULT_CACHE_DIR / "single_combo_row_cache_rank_pattern_prior_date_train_surface.json"
)
DEFAULT_OUTPUT = (
    DEFAULT_CACHE_DIR
...
def _live_ranked_cache(
    *,
    ...
    for feature_name in rank_features:
...
def _load_training_state(
    *,
    ...
    if not model_spec_name or not selector_spec_name:
        raise ValueError("train surface is missing model_spec or selector_spec")
    model_spec, selector_spec = train_surface._resolve_best_specs(
    ...
    answers_by_window, _window_context, row_cache = train_surface._train_answers_by_window(
    ...
    rank_features = tuple(str(name) for name in surface_payload["rank_feature_names"])
    combo_features = tuple(str(name) for name in surface_payload["combo_feature_names"])
    ...
    ranked_cache, hit_cache = _build_rank_and_hit_caches(
        ...
        race_ids=race_ids,
        answers={str(race_id): list(answer[:3]) for race_id, answer in answers.items()},
    ...
    policies = selector_base._selected_policies(
        hit_cache=hit_cache,
        train_indices=train_indices,
        patterns=patterns,
        top_n=model_spec.top_n,
        metric="exact",
    )
    train_source = {
    ...
    train_rows_by_race = selector_base._rows_by_race_for_policies(
        race_ids=sorted(train_source),
        rows_by_race=rows_by_race,
        fallback_predictions=train_source,
        policies=policies,
        ranked_cache=ranked_cache,
        combo_features=combo_features,
        answers=answers,
    )
    train_rows = selector_base._flatten_candidate_rows(train_rows_by_race)
    x_train, y_train = selector_base._matrix(train_rows, target=model_spec.target)
    model = selector_base._fit_model(matrix=x_train, labels=y_train, spec=model_spec)
    return {
        "combo_features": combo_features,
        "model": model,
        ...
        "train_label_unique_count": len(set(np.round(y_train, 6).tolist())),
    }
...
def _predict_live_window(
    *,
    ...
    policies = train_state["policies"]
    combo_features = train_state["combo_features"]
    ...
    live_ranked_cache = _live_ranked_cache(
        ...
        rank_features=train_state["rank_features"],
    )
    live_rows_for_scoring = selector_base._rows_by_race_for_policies(
        race_ids=sorted(race_ids),
        rows_by_race=live_rows_by_race,
        fallback_predictions=current_best_predictions,
        policies=policies,
        ranked_cache=live_ranked_cache,
        combo_features=combo_features,
        answers={},
    )
    eval_rows = selector_base._flatten_candidate_rows(live_rows_for_scoring)
    x_eval, _ = selector_base._matrix(eval_rows, target=model_spec.target)
    scores = selector_base._model_scores(
        model=train_state["model"],
        matrix=x_eval,
        spec=model_spec,
    )
    score_map = {
        (str(row["race_id"]), row["combo"]): float(score)
        for row, score in zip(eval_rows, scores, strict=False)
    }
    selected, selection_diagnostics = selector_base._select_predictions(
        candidate_rows_by_race=live_rows_for_scoring,
    ...
    candidate_counts = [
        float(len(live_rows_for_scoring.get(race_id, []))) for race_id in race_ids
    ]
    ...
    diagnostics = {
        ...
        "feature_contract": "row_cache_rank_pattern_prior_date_live_rows_no_answer",
        ...
        "selector": f"{model_spec.name}/{selector_spec.name}",
        ...
        "selected_source_exact_rate_mean": round(
            float(selection_diagnostics.get("selected_source_exact_rate_mean", 0.0)),
            6,
        ),
...
def _recommended_next_action(
    *,
    ...
    if train_surface_status != "passed":
        return {
            "action": "repair_row_cache_rank_pattern_train_surface_before_live_port",
            "blocking": False,
            "classification": "background_modeling_candidate",
            "queue_priority_score": 94.85,
            "reason": (
                "The live scorer needs the frozen row-cache rank-pattern train "
                "surface to rebuild the prior-date HGB history."
            ),
        }
...
def build_artifact(
    *,
    ...
    if pre_status is not None:
        ...
        status = "blocked_missing_row_cache_rank_pattern_train_surface"
    else:
    ...
    if status == "passed":
        recommended = {
            "action": "materialize_locked_best_next_parent_after_row_cache_rank_pattern_prior_date",
            "blocking": False,
            "classification": "background_modeling_candidate",
            "queue_priority_score": 94.83,
            "reason": (
                "The locked row-cache rank-pattern selector emitted complete "
