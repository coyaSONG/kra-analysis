"""Materialize or strictly block live entity-history pair-overlay source."""
...
DEFAULT_SOURCE_TARGET = DEFAULT_CACHE_DIR / (
    "clean_release_current_best_entity_history_pair_overlay_prior_date_"
    ...
    *,
    ...
    if live_source_context.get("status") != "passed":
        ...
        if isinstance(child_action, str) and child_action:
            return {
                ...
                "queue_priority_score": float(
                    child_recommended.get("queue_priority_score", 94.9)
                ),
                "reason": (
                    child_recommended.get("reason")
                    or "The entity-history overlay source reported a downstream live-port dependency."
                ),
        ...
        return {
            ...
            "reason": (
                "The entity-history pair-overlay source cannot emit predictions "
                "until the entity-history overlay prior-date source is "
                "materialized with the exact contract."
            ),
        }
    if candidate_status != "passed":
        return {
            "action": "repair_live_candidate_features_before_entity_history_pair_overlay_prior_date_port",
            "blocking": False,
            "classification": "background_modeling_candidate",
            "queue_priority_score": 94.88,
            "reason": (
                "The entity-history pair-overlay source needs complete live full-combo "
                "candidate features and live records before pass-through can be trusted."
            ),
        }
    if ready_to_pass_through:
        return {
            "action": "materialize_locked_best_row_cache_rank_pattern_from_passed_entity_history_pair_overlay",
            "blocking": False,
            "classification": "background_modeling_candidate",
            "queue_priority_score": 94.86,
            "reason": (
                "The fallback-only entity-history pair-overlay source has been "
                "materialized and can now unblock the row-cache rank-pattern parent."
            ),
        }
    return {
        "action": "inspect_entity_history_pair_overlay_prior_date_pass_through_coverage_gap",
        "blocking": False,
        "classification": "background_modeling_candidate",
        "queue_priority_score": 94.84,
        "reason": (
            "The entity-history pair-overlay source is present, but its live "
...
def build_artifact(
    *,
    ...
    if ready_to_pass_through:
        ...
        status = "blocked_entity_history_pair_overlay_prior_date_coverage_gap"
        coverage = {
            "status": "blocked",
            "expected_race_count": expected_race_count,
            "predicted_race_count": predicted_race_count,
        ...
        predictions = {}
    return {
        "format_version": "single-combo-live-entity-history-pair-overlay-prior-date-materializer-v1",
