...
class PromptStructure:
    ...
    def get_section(self, tag: str) -> Optional[PromptSection]:
        """특정 태그의 섹션 반환"""
    ...
    def add_section(self, tag: str, content: str, order: Optional[int] = None) -> None:
        """새 섹션 추가"""
        if order is None:
            order = len(self.sections)
        self.sections[tag] = PromptSection(tag=tag, content=content, order=order)
...
class RequirementsEditor:
    ...
    def add_requirement(self, requirement: str, position: Optional[int] = None) -> None:
        """새 요구사항 추가"""
