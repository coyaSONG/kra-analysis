...
from .prompt_parser import PromptStructure, PromptSection
...
"""
    ...
    def optimize_prompt(self, structure: PromptStructure) -> Tuple[PromptStructure, List[Change]]:
        """전체 프롬프트 최적화"""
        changes = []
    ...
    def _optimize_text_content(self, structure: PromptStructure) -> List[Change]:
        ...
        for section_tag, section in structure.sections.items():
    ...
    def _optimize_structure(self, structure: PromptStructure) -> List[Change]:
        ...
        for section_tag, section in structure.sections.items():
            if section.content.strip() in section_contents:
                # 중복 내용 발견
                duplicate_of = section_contents[section.content.strip()]
                # 섹션 제거 또는 통합 로직
                # (실제 구현에서는 더 신중하게 처리)
        ...
        for section_tag, section in structure.sections.items():
            ...
            for full_term, abbrev in abbreviations.items():
                ...
                if full_term in content:
                    ...
                    if len(parts) == 2:
                        new_content = parts[0] + full_term + parts[1].replace(full_term, abbrev)
